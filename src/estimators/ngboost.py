...
class NGBRegressorWrapper(BaseEstimator):
    ...
    def __init__(self,
                 ...
                 early_stopping_rounds: bool = None,
                 ):
    ...
    def fit(self, X, y):
        ...
        base_estimator = DecisionTreeRegressor(criterion=self.base_criterion, splitter=self.base_splitter,
        ...
        ngb = NGBRegressor(Dist=dists[self.dist], Score=scores[self.score], Base=base_estimator,
                           natural_gradient=self.natural_gradient, n_estimators=self.n_estimators,
                           learning_rate=self.learning_rate, minibatch_frac=self.minibatch_frac,
                           col_sample=self.col_sample, verbose=self.verbose, verbose_eval=self.verbose_eval,
                           tol=self.tol, random_state=self.random_state, validation_fraction=self.validation_fraction,
