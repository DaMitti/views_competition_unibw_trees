"""
Mostly simple helper functions for conversions between data index IDs and real-life formats and prediction cols to
xarrays.
"""
...
@cache
def get_date(month_id: int) -> pendulum.datetime:
    """ converts UCDP's month_id to datetime """
    year = 1980 + math.floor((month_id-1)/12)
    month = month_id%12 if month_id%12 != 0 else 12
    return pendulum.datetime(year, month, 1)

@cache
def get_month_id(date: datetime.date|datetime.datetime|pendulum.Date|pendulum.DateTime) -> int:
