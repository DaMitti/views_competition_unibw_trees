"""
Different functions to generate the semi-automated cluster assignments of the grid cells for our local models.
Implemented are two automated methods, DBSCAN and HDBSCAN, as well as cluster assignment based on UN statistical
regions and support for the pipeline's test mode.
"""
...
def make_clusters_unregions(df: pd.DataFrame, s: float = 0.5, visualize_clusters: bool = False) -> pd.DataFrame:
    """
    Information on the source files used for country/region matching (file paths are hardcoded):
    Iso3 Matching: GeoBoundaries (https://www.geoboundaries.org/) Comprehensive Global Administrative Zones (CGAZ) ADM0
                   Not in repo - needs to be downloaded to regenerate.
    Areas: United Nations Statistics Division: Standard country or area codes for statistical use (M49) - Sub-regions
           Corresponding csv downloaded from: https://unstats.un.org/unsd/methodology/m49/overview/

    Args:
        df (pd.DataFrame): Dataframe with data used for clustering
        s (float): grid size (should not be changed unless for other applications)
    ...
    def get_neighbors_indices(id):
        ...
        ys = [y - s, y, y + s]
        xs = [x - s, x, x + s]
        index_list = [(ys[0], xs[0]), (ys[0], xs[1]), (ys[0], xs[2]),
                      (ys[1], xs[0]), (ys[1], xs[2]),
                      (ys[2], xs[0]), (ys[2], xs[1]), (ys[2], xs[2])]
        # handle edge cases
    ...
    if os.path.exists('data/region_matching.parquet'):
        ...
        countries.geometry = countries.geometry.make_valid() # takes a bit but avoids annoying erros
        # UNSD regions - using Intermedia Regions
        regions = pd.read_csv(
            'data/UNSDMethodology.csv', delimiter=';', usecols=['Country or Area','ISO-alpha3 Code', 'Intermediate Region Name', 'Sub-region Name']
        )
        regions['region'] = regions.apply(
            lambda x: x['Sub-region Name'] if pd.isna(x['Intermediate Region Name']) else x['Intermediate Region Name'],
            axis=1
        )
        regions = regions.rename(columns={'Country or Area':'country','ISO-alpha3 Code':'iso3'}).drop(
            columns=['Intermediate Region Name', 'Sub-region Name']
        )
        ...
        df_clusters.region = df_clusters.apply(lambda x: region_check(x['region'], x['priogrid_gid']), axis=1)
        ...
        df_clusters = df_clusters.replace(
            to_replace=['Southern Europe', 'Eastern Europe', 'Central Asia','Southern Asia'],
            value='Western Asia'
        )
        df_clusters['region'] = df_clusters['region'].str.replace(' ', '_').lower()

        # save
        df_clusters = df_clusters.drop(columns=['iso3', 'country']).set_index('priogrid_gid').rename(columns={'region': 'cluster'})
    ...
    if visualize_clusters:
        ...
        df_test['geometry'] = df_test.coords.apply(lambda x: box(x[1] - .25, x[0] - .25, x[1] + .25, x[0] + .25))
...
def make_clusters_dbscan(df: pd.DataFrame, cluster_kwargs: dict, visualize_clusters: bool = False) -> pd.DataFrame:
    """
    Semi-automated clustering with DBSCAN based on spatial distribution of grid cells with violence across input data
    (disregarding time) and manually set parameters for the algorithm. Grid cells without violence or not initially
    assigned are assigned based on the distance to polygons drawn around identified clusters.
    ...
    def assign_zeros_to_clusters():
        ...
        coords = np.concatenate((clust_df[clust_df['ged_sb'] == 0][['lon', 'lat']].to_numpy(), points_violence[labels_violence == -1]))
        pgids = np.concatenate((clust_df[clust_df['ged_sb'] == 0].index.to_numpy(), pgids_violence[labels_violence == -1]))
    ...
    if visualize_clusters:
        ...
        clust_df.set_index(['lat', 'lon'])['cluster'].to_xarray().plot(ax=ax, cmap='nipy_spectral',
...
def make_clusters_hdbscan(df: pd.DataFrame, min_months: int, cluster_kwargs: dict,
                          visualize_clusters: bool = False) -> pd.DataFrame:
    """
...
def make_clusters_test() -> pd.DataFrame:
    """ clusters for pipeline test mode """
    df = create_dummy_data(2017)
    clust_df = pd.DataFrame(index=df.index.levels[1])
