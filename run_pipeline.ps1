...
function Start-Predict {
    ...
    if ($container_id) {
        ...
        $container_id=$(docker run -d -v ${PWD}:/usr/src/app ccew-tree "(cd src && PYTHONPATH=../ python3.11 -u competition_pipeline.py)")
    }
    if ($logs) {
...
function Start-Evaluate {
    ...
    if ($container_id) {
        ...
        $container_id=$(docker run -d -v ${PWD}:/usr/src/app ccew-tree "(cd src && PYTHONPATH=../ python3.11 -u evaluation_pipeline.py)")
    }
    if ($logs) {
        docker logs $container_id -f
        exit 0
